...
function ModifierExpediteurContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      expediteursMemeFamille &&
      expediteursMemeFamille.length > 0 &&
      !(expediteursMemeFamille.length === 1 && expediteursMemeFamille[0].id === id)
    ) {
      setError("Cet email d'expéditeur est déjà utilisé");
      return;
    }
    if (email !== initialEmail) {
      // Email changé : reset vérification, token, dates, envoi mail
      const token = uuidv4();
      const now = new Date();
  ...
  return (
    <AppLayout>
      <Suspense fallback={
        <div className="flex justify-center min-h-screen bg-gray-50 p-8">
          <div className="bg-white rounded-2xl shadow-lg flex flex-col md:flex-row w-full max-w-4xl p-0 overflow-hidden">
            <div className="flex-1 w-full md:max-w-lg space-y-8 p-10">
